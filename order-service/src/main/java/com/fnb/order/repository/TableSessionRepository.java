...
public interface TableSessionRepository extends JpaRepository<TableSession, UUID> {
    ...
    @Query("SELECT s FROM TableSession s WHERE s.status = 'ACTIVE' AND NOT EXISTS (SELECT o FROM Order o WHERE o.session = s AND o.status NOT IN ('PAID', 'CANCELLED')) AND EXISTS (SELECT o2 FROM Order o2 WHERE o2.session = s) AND (SELECT MAX(o3.updatedAt) FROM Order o3 WHERE o3.session = s) < :limit")
    List<TableSession> findSessionsReadyForCleanup(@Param("limit") LocalDateTime limit);

    @Query("SELECT s FROM TableSession s WHERE s.status = 'ACTIVE' AND s.openedAt < :limit AND NOT EXISTS (SELECT o FROM Order o WHERE o.session = s AND o.total > 0)")
    List<TableSession> findEmptySessions(@Param("limit") LocalDateTime limit);

    @Query("SELECT s FROM TableSession s WHERE s.status = 'ACTIVE' AND s.openedAt < :limit AND EXISTS (SELECT o FROM Order o WHERE o.session = s AND o.status = 'OPEN')")
